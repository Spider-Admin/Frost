...
public class Execute {
	 ...
	 * @return ExecResult the output generated by the program. Standard ouput and
	 *         Error output are captured.
	 */
	public static ExecResult run_wait(String[] cmd, String cs) {
		...
		try {
			...
			try (InputStream isStdOut = result.proc.getInputStream();
			...
			try (InputStream isStdErr = result.proc.getErrorStream();
					InputStreamReader iSReader = new InputStreamReader(isStdErr, cs);
					BufferedReader reader = new BufferedReader(iSReader);) {
				int count = 0;
				while (count != -1) {
					count = reader.read(cbuf, 0, BUFF_SIZE);
